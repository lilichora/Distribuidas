...
const Usuarios = () => {
    ...
    const handleSave = async () => {
        try {
            if (editing) {
                await axios.put(`http://localhost:8001/api/usuarios/editar/${currentUsuario.id}`, currentUsuario);
            } else {
                await axios.post('http://localhost:8001/api/usuarios/crear', currentUsuario);
            }
            fetchUsuarios();
            handleClose();
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:8001/api/usuarios/eliminar/${id}`);
            fetchUsuarios();
        } catch (error) {
